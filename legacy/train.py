...
import simCLR_unet_brats.loss.BCEDiceLoss as BCEDiceLoss
...
loss_names = list(BCEDiceLoss.__dict__.keys())
    ...
    with torch.no_grad():
        for i, (input, target) in tqdm(enumerate(val_loader), total=len(val_loader)):
            ...
            if args.deepsupervision:
                ...
                output = model(input)
                loss = criterion(output, target)
                iou = iou_score(output, target.long())
...
def main():
    ...
    if args.loss == 'BCEWithLogitsLoss':
        ...
        criterion = BCEDiceLoss.__dict__[args.loss]().cuda()
    ...
    train_dataset = BratsTrainDataset(args, train_img_paths, train_mask_paths, args.aug)
    val_dataset = BratsTrainDataset(args, val_img_paths, val_mask_paths)
